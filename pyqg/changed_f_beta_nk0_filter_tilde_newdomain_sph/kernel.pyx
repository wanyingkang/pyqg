...
cdef class PseudoSpectralKernel:
    ...
    # restoring strength
    cdef public DTYPE_com_t [:, :, :] gamma
    cdef public DTYPE_real_t H0
    ...
    def __init__(self, int nz, int ny, int nx, int nk0, int fftw_num_threads=1):
        ...
        self.nk = nx/2 + 1
        self.nk0 = nk0
        self.a = np.zeros((self.nz, self.nz,self.nl, self.nl, self.nk0), DTYPE_com)
        z = np.zeros((self.nz),DTYPE_real)
        self.z = z 
    ...
    cdef void __invert(self) nogil:
        ...
        cdef Py_ssize_t k, k1, k2, j, i, j1
        cdef DTYPE_com_t [:, :, :] qhh
        cdef DTYPE_real_t [:,:,:] p

	# Wanying Kang: Add BC
        with gil:
            p=np.zeros((self.nz,self.ny,self.nx), dtype=DTYPE_real)
            qhh = self.qh
            #for k in range(self.nz):
            #    qhh[k,int(self.nl/2),0]=0.
            #    for i in range(1,self.nk):
            #        qhh[k,int(self.nl/2),i]=0.
            #        qhh[k,int(self.nl*3/4),i]=0.
                 
        # set ph to zero
        #with gil:
        #    print np.any(np.asarray(self.qh)!=0)
        for k in range(self.nz):
...
#                            self.a[k2,k1,j,i] * self.qh[k1,j,i] )
        ...
        # Wanying Kang: as assumed when adding BCs, ph's -nl/2 and nl/2-1 modes are zero
        #with gil:
        #    for k in range(self.nz):
        #        for i in range(self.nx):
        #            self.ph[k,int(self.nl/2),i]=0.
        #            self.ph[k,int(self.nl*3/4),i]=0.
	# check if the bcelim method (replace some modes' equations by BCs) works
        #with gil:
        #    p = self.ifft(np.asarray(self.ph))
        #    print(np.asarray(p[3,self.ny-1,:]))
        #    print(np.asarray(p[7,0,:]))
    ...
    cdef void __do_advection(self) nogil:
        ...
        for k in range(self.nz):
            for j in prange(self.ny, nogil=True, schedule='static',
                      ...
                      num_threads=self.num_threads):
                for i in range(self.nx):
                    self.uq[k,j,i] = (self.u[k,j,i]+self.Ubg[k]) * self.q[k,j,i] / (self.coslat[j,i])**2 / (self.sinlat[j,i]) * exp(self.z[k]/2./self.H0) 
                    self.vq[k,j,i] = self.v[k,j,i] * self.q[k,j,i] / (self.coslat[j,i])**2 / (self.sinlat[j,i]) * exp(self.z[k]/2./self.H0)
            ...
            for j in range(self.nl):
                for i in range(self.nk):
                    ...
                    self.dqhdt[k,j,i] = -( self._ik[i] * self.uqh[k,j,i] +
                                    self._il[j] * self.vqh[k,j,i])  + self.Fqh[k,j,i]
                    for j1 in range(self.nl):
                        self.dqhdt[k,j,i] = self.dqhdt[k,j,i] - self._ikQy[k,j,j1,i] * self.ph[k,j1,i] + self.gamma[k,j,j1] * (self.qtarh[k,j1,i]-self.qh[k,j1,i])
        return
