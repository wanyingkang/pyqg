...
cdef class PseudoSpectralKernel:
    ...
    cdef DTYPE_real_t [:, :, :] q
    cdef DTYPE_com_t [:, :, :] qh
    cdef DTYPE_real_t [:, :, :] Fq
    cdef DTYPE_com_t [:, :, :] Fqh
    cdef DTYPE_real_t [:, :, :] qtar
    cdef DTYPE_com_t [:, :, :] qtarh
    # streamfunction
    cdef DTYPE_com_t [:, :, :] ph
    # velocities
    cdef DTYPE_real_t [:, :, :] u
    ...
    cdef public DTYPE_real_t [:, :] coslat
    cdef public DTYPE_real_t [:, :] sinlat
    cdef DTYPE_real_t damp
    ...
    cdef public DTYPE_real_t dy
    # the variables needed for inversion and advection
    # store a as complex so we don't have to typecast in inversion
    # Wanying change a dimension from 4 to 5
    cdef DTYPE_com_t [:, :, :, :, :] a
    # Wanying add qh0 and qhN
    cdef public DTYPE_com_t [:, :, :] qh0
    cdef public DTYPE_com_t [:, :, :] qhN
    ...
    # restoring strength
    cdef public DTYPE_real_t gamma
    cdef public DTYPE_real_t tau
    ...
    cdef object fft_qtar_to_qtarh
    cdef object fft_Fq_to_Fqh
    cdef object ifft_qh_to_q
    cdef object ifft_qtarh_to_qtar
    cdef object ifft_Fqh_to_Fq
    ...
    def __init__(self, int nz, int ny, int nx, int nk0, int fftw_num_threads=1):
        self.nz = nz
        self.ny = ny
        self.nx = nx
        self.nl = ny
        self.nk = nx/2 + 1
        self.nk0 = nk0
        self.a = np.zeros((self.nz, self.nz,self.nl, self.nl, self.nk), DTYPE_com)
        self.qh0 = np.zeros((self.nz, self.nl, self.nk), DTYPE_com)
        self.qhN = np.zeros((self.nz, self.nl, self.nk), DTYPE_com)
        ...
        dy = 0.
        self.dy = dy
        coslat = np.zeros((self.nl,self.nk),DTYPE_real)
        self.coslat=coslat
        sinlat = np.zeros((self.nl,self.nk),DTYPE_real)
        self.sinlat=sinlat
        #coslat1 = np.zeros(self.nl,DTYPE_real)
        #self.coslat1=coslat1
        #sinlat1 = np.zeros(self.nl,DTYPE_real)
        #self.sinlat1=sinlat1
        q = self._empty_real()
        self.q = q
        qh = self._empty_com()
        self.qh = qh
        qtar = self._empty_real()
        self.qtar = qtar
        qtarh = self._empty_com()
        self.qtarh = qtarh
        Fq = self._empty_real()
        self.Fq = Fq
        Fqh = self._empty_com()
        ...
        IF PYQG_USE_PYFFTW:
            ...
            self.fft_Fq_to_Fqh = pyfftw.FFTW(Fq, Fqh, threads=fftw_num_threads,
                             direction='FFTW_FORWARD', axes=(-2,-1))
            self.fft_qtar_to_qtarh = pyfftw.FFTW(qtar, qtarh, threads=fftw_num_threads,
                    direction='FFTW_FORWARD', axes=(-2,-1))
            self.ifft_qh_to_q = pyfftw.FFTW(qh, q, threads=fftw_num_threads,
                             direction='FFTW_BACKWARD', axes=(-2,-1))
            self.ifft_Fqh_to_Fq = pyfftw.FFTW(Fqh, Fq, threads=fftw_num_threads,
                             direction='FFTW_BACKWARD', axes=(-2,-1))
            self.ifft_qtarh_to_qtar = pyfftw.FFTW(qtarh, qtar, threads=fftw_num_threads,
                             direction='FFTW_BACKWARD', axes=(-2,-1))
            # Wanying Kang change the expression for u,v. u=-psi_y/coslat^2, v=psi_x/coslat^2
            # /np.tile(self.coslat[np.newaxis,:,:]**2,(self.nz,1,1))
    ...
    IF PYQG_USE_PYFFTW==0:
        ...
        def fft_qtar_to_qtarh(self):
            self.qtarh = npfft.rfftn(self.qtar, axes=(-2,-1))
        def fft_Fq_to_Fqh(self):
            self.Fqh = npfft.rfftn(self.Fq, axes=(-2,-1))
        def ifft_qh_to_q(self):
            self.q = npfft.irfftn(self.qh, axes=(-2,-1))
        def ifft_Fqh_to_Fq(self):
            self.Fq= npfft.irfftn(self.Fqh, axes=(-2,-1))
        def ifft_qtarh_to_qtar(self):
            self.qtar= npfft.irfftn(self.qtarh, axes=(-2,-1))
        # Wanying Kang change the expression for u,v. u=-psi_y/coslat^2, v=psi_x/coslat^2
        # /np.tile(self.coslat[np.newaxis,:,:]**2,(self.nz,1,1))
    ...
    cdef void __invert(self) nogil:
        ...
        cdef Py_ssize_t k, k1, k2, j, i, j1, iit, nit
        cdef DTYPE_real_t [:, :] qmod, qmodold, dqmod, pk20, pk2N
        cdef DTYPE_com_t [:, :, :] qhh
        cdef DTYPE_real_t  p0 , pN
...
#                            self.a[k2,k1,j,i] * self.qh[k1,j,i] )
        ...
        # Wanying add modification term to qh to change BC
        # qmod = (self.p[:,self.ny-1,:]-self.p[:,0,:])/(self.dy**2) 
        # qhh = ( qh + qmod * qh0 - qmod * qhN - k^2*ph)

        # Number of iterations, at t=0, the initial psi is wrong, so need more iterations
        if self.t==0:
            nit=100
        else:
            nit=1
        ...
        pN = 0.
        with gil:
            qmod = np.zeros((self.nz,self.nx), dtype=DTYPE_real)
            qmodold  = qmod
            dqmod = qmod
            pk20 = qmod
            pk2N = qmod
        ...
        for iit in range(nit):
            #with gil:
                #print("iit: {}".format(iit))
            # Deal with boundary condition for V
                ...
                for i in range(1,self.nk):
                    ...
                    with gil:
                        for j in range(self.nl):    
                            pk20[k,i] = pk20[k,i] + np.real(self.ph[k,j,i]*self.kk[i]**2)
                            pk2N[k,i] = pk2N[k,i] + np.real(self.ph[k,j,i]*self.kk[i]**2 * np.exp(2.0j*np.pi*j*(self.nl-1)/self.nl))
                    pk20[k,i] = pk20[k,i]/self.nl/self.nk
                    pk2N[k,i] = pk2N[k,i]/self.nl/self.nk
            for k in range (self.nz):
                for j in prange(self.nl, nogil=True, schedule='static',
                         chunksize=self.chunksize,
                         num_threads=self.num_threads):
                    for i in range(1,self.nk):
                        qhh[k,j,i] = self.qh[k,j,i]  - pk20[k,i] * self.qh0[k,j,0] - pk2N[k,i] * self.qhN[k,j,0]
            ...
            for k in range(self.nz):
                ...
                with gil:
                    for j in range(self.nl):    
                        p0 = p0 + np.real(self.ph[k,j,0])
                        pN = pN + np.real(self.ph[k,j,0] * np.exp(2.0j*np.pi*j*(self.nl-1)/self.nl))
                p0 = p0/self.nl
                pN = pN/self.nl
                qmod[k,0] = (pN-p0)/(self.dy**2)/self.nk
            #with gil:
            #    print("qmod")
            #    print(np.asarray(qmod)[np.asarray(qmod)!=0])
            #    print("qh0")
            #    print(np.asarray(self.qh0)[np.asarray(self.qh0)!=0])
            for k in range (self.nz):
                for j in prange(self.nl, nogil=True, schedule='static',
                         chunksize=self.chunksize,
                         num_threads=self.num_threads):
                    qhh[k,j,0] = qhh[k,j,0] + qmod[k,0] * self.qh0[k,j,0] - qmod[k,0] * self.qhN[k,j,0]
            ...
            # Wanying Kang. Adapt to 5-d tensor inversion matrix.
            for k2 in range(self.nz):
                for k1 in range(self.nz):
                    for j in prange(self.nl, nogil=True, schedule='static',
                              chunksize=self.chunksize,
                        ...
                        for j1 in range(self.nl):
                            for i in range(self.nk):
                                self.ph[k2,j,i] = ( self.ph[k2,j,i] +
                                    self.a[k2,k1,j,j1,i] * qhh[k1,j1,i] )
        ...
        for k in range(self.nz):
            for j in prange(self.ny, nogil=True, schedule='static',
                ...
                for i in range(self.nx):
                    self.uq[k,j,i] = (self.u[k,j,i]+self.Ubg[k]) * self.q[k,j,i] / (self.coslat[j,i])**2 
                    self.vq[k,j,i] = self.v[k,j,i] * self.q[k,j,i] / (self.coslat[j,i])**2
        ...
        for k in range(self.nz):
            ...
            for j in range(self.nl):
                for i in range(self.nk):
                    ...
                    # Wanying add linear restoring term with time scale specified
                    # in self.gamma, and target qh in self. qtarh.
                    # Extra forcing needed to maintain the background, self.Fqh
                    self.dqhdt[k,j,i] = -( self._ik[i] * self.uqh[k,j,i] +
                                    self._il[j] * self.vqh[k,j,i] 
                                ) + self.gamma * (self.qtarh[k,j,i]-self.qh[k,j,i]) + self.Fqh[k,j,i]
                    for j1 in range(self.nl):
                        self.dqhdt[k,j,i] = self.dqhdt[k,j,i] - self._ikQy[k,j,j1,i] * self.ph[k,j1,i]
        return
                ...
                for i in range(self.nk):
                    self.dqhdt[k,j,i] = (
                     self.dqhdt[k,j,i] +
                             ...
                             self._k2l2[j,i] *
                             self.ph[k,j,i]) )
        # Wanying Kang: add 4-order damping to y direction
        for k in range(self.nz):
            for j in prange(self.nl, nogil=True, schedule='static',
                      chunksize=self.chunksize,
                      num_threads=self.num_threads):
                for i in range(self.nk):
                    self.dqhdt[k,j,i] = (self.dqhdt[k,j,i]
                            - self.damp * ((self.ll[j])**4) * self.qh[k,j,i])
        #with gil:
            #print(self.damp * ((self.ll[30])**4.) * self.qh[1,30,1])
            #print(self.dqhdt[1,30,1])
        return
    ...
    cdef void __forward_timestep(self) nogil:
        ...
        # Wanying Kang: filter out k>nk0 waves
        if self.nk0<self.nk:
            for k in range(self.nz):
                for j in prange(self.nl, nogil=True, schedule='static',
                        chunksize=self.chunksize,
                        num_threads=self.num_threads):
                    for i in range(self.nk0,self.nk,1):
                        qh_new[k,j,i]=0.0
                        self.qh[k,j,i]=0.0
    ...
    property ll:
        ...
        def __set__(self, np.ndarray[DTYPE_real_t, ndim=1] l):
            ...
            for j in range(self.nl):
                for i in range(self.nk):
                    self._k2l2[j,i] = self.kk[i]**2 + self.ll[j]**2
    # Wanying Kang, Add damp property
    property damp:
        def __get__(self):
            return np.asarray(self.damp)
        def __set__(self, damp):
            self.damp = damp
    ...
    property a:
        ...
        # WHY? Wanying
        # Wanying set ndim from 4 to 5
        def __set__(self, np.ndarray[DTYPE_com_t, ndim=5] b):
    ...
    property Qy:
        ...
        def __set__(self, np.ndarray[DTYPE_com_t, ndim=3] Qy):
            self.Qy = Qy
            #self._ikQy = 1j * (np.asarray(self.kk)[np.newaxis, :] *np.asarray(Qy)[:, np.newaxis])
            self._ikQy = np.asarray(self.Qy)[:,:,:,np.newaxis]*1j*np.asarray(self.kk)[np.newaxis,np.newaxis,np.newaxis,:]
    property q:
        def __get__(self):
    ...
    property qtar:
        def __get__(self):
            return np.asarray(self.qtar)
        def __set__(self, np.ndarray[DTYPE_real_t, ndim=3] b):
            cdef  DTYPE_real_t [:, :, :] b_view = b
            self.qtar[:] = b_view
            self.fft_qtar_to_qtarh()
    property qtarh:
        def __get__(self):
            return np.asarray(self.qtarh)
        def __set__(self, np.ndarray[DTYPE_com_t, ndim=3] b):
            cdef  DTYPE_com_t [:, :, :] b_view = b
            self.qtarh[:] = b_view
            self.ifft_qtarh_to_qtar()
            # input might have been destroyed, have to re-assign
            self.qtarh[:] = b_view
    property Fq:
        def __get__(self):
            return np.asarray(self.Fq)
        def __set__(self, np.ndarray[DTYPE_real_t, ndim=3] b):
            cdef  DTYPE_real_t [:, :, :] b_view = b
            self.Fq[:] = b_view
            self.fft_Fq_to_Fqh()
    ...
    #property coslat:
    #    def __get__(self):
    #        return np.asarray(self.coslat)
    #property sinlat:
    #    def __get__(self):
    #        return np.asarray(self.sinlat)
    #property coslat1:
    #    def __get__(self):
    #        return np.asarray(self.coslat1)
    #property sinlat1:
    #    def __get__(self):
    #        return np.asarray(self.sinlat1)
