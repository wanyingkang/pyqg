...
cdef class PseudoSpectralKernel:
    ...
    cdef public DTYPE_real_t [:,:] pert
    cdef public DTYPE_real_t omega
    ...
    def __init__(self, int nz, int ny, int nx, int fftw_num_threads=1):
        ...
        self.rek = 0.0
        self.omega = 0.0
        self.pert = np.zeros((self.nz,self.nk),dtype=DTYPE_real)
            ...
            for j in prange(self.nl, nogil=True, schedule='static',
                ...
                for i in range(self.nk):
                    ...
                    self.dqhdt[k,j,i] = -( self._ik[i] * self.uqh[k,j,i] +
                                    ...
                                    self._ikQy[k,i] * self.ph[k,j,i] + self._ilQx[k,j] * self.ph[k,j,i])
        return
            ...
            for i in range(self.nk):
                self.dqhdt[k,j,i] = (
                 self.dqhdt[k,j,i] +
                         ...
                         self._k2l2[j,i] *
                         self.ph[k,j,i])
                         + self.pert[k,i] * cos(self.omega*self.t))
        return
    ...
    property Vbg:
        def __get__(self):
            return np.asarray(self.Vbg)
        def __set__(self, np.ndarray[DTYPE_real_t, ndim=1] Vbg):
            self.Vbg = Vbg
    property Qy:
        def __get__(self):
    ...
    property vfull:
        def __get__(self):
            return np.asarray(self.v) +\
	     np.asarray(self.Vbg)[:,np.newaxis,np.newaxis]
    property uh:
        def __get__(self):
