...
class Model(PseudoSpectralKernel):
    ...
    a : real array
        inversion matrix (`nz`, `nz`, `nl`, `nl`, `nk`) (cython)
    q : real array
        Potential vorticity in real space (`nz`, `ny`, `nx`) (cython)
    qh : complex array
        Potential vorticity in spectral space (`nz`, `nl`, `nk`) (cython)
    ph : complex array
        Streamfunction in spectral space (`nz`, `nl`, `nk`) (cython)
    u, v : real array
    ...
    def __init__(
        ...
        re=6371000.,
        latmax=80.,
        nk0=33,                     # spectra cut-off in x direction. nx/2+1
        nz=1,
        nx=64,                     # grid resolution
        ny=None,
        # timestepping parameters
        dt=300.,                   # numerical timestep
        twrite=300.,               # interval for cfl and ke writeout (in timesteps)
        ...
        damp = 1.e7,                 # damp coefficient for damping (-damp*(l)^4*q)
        ...
        ntd : int
            ...
            your machine.
        """
        L=re*2*pi                 # domain size is L [m]
        W=2.*re*np.log((1+np.sin(latmax/180.*pi)/np.cos(latmax/180.*pi)))
        ...
        PseudoSpectralKernel.__init__(self, nz, ny, nx, nk0, ntd)
        self.L = L
        self.W = W
        self.re = re
        self.latmax = latmax
    ...
    def stability_analysis(self,bottom_friction=False):
        ...
        """
        ## Wanying: after adding lat dependence to beta, we cannot use this stability analysis any more. Modification needed!
    ...
    def _initialize_grid(self):
        ...
        self.x,self.y = np.meshgrid(
            ...
            np.arange(0.5,self.ny,1.)/self.ny*self.W-self.W/2. )
        # self.x and self.y are not used. only used when calculating self.lat.
        self.z = np.arange(0.5,self.nz,1.)*self.Hi[0]   # only valid for evenly distributed layers.

        # Calculate corresponding latitude and coslat, sinlat
        self.lat = np.array(2.*np.arctan(np.exp(self.y/self.re))-pi/2.)
        if self.newdomain==1:
            # Wanying Kang: New domain two northen hemispheres connect together 
            self.lat[:int(self.ny/2),:]=self.lat[int(self.ny/2):,:]
            self.lat[int(self.ny/2):,:]=self.lat[int(self.ny)-1:int(self.ny/2)-1:-1,:]
        elif self.newdomain==2:
            self.lat[:int(self.ny/2),:]=self.lat[int(self.ny/4):int(self.ny/4*3),:]
            self.lat[int(self.ny/2):,:]=self.lat[int(self.ny/2)-1:None:-1,:]
            
        self.sinlat=np.sin(self.lat)
        self.coslat=np.cos(self.lat)
        self.sinlat1=np.squeeze(self.sinlat[:,1])
        self.coslat1=np.squeeze(self.coslat[:,1])
        ...
        self.ll = self.dl*np.append( np.arange(0.,self.nl/2),
            np.arange(-self.nl/2,0.) )
    ...
    #    # Wanying Kang add qh0 and qhN pass to kernel
    #    self._kernel_init(
    ...
    def set_qtar(self,qtar):
        self.qtar = qtar
        self.qtarh = self.fft(qtar)

    def set_Fq(self,Fq):
        self.Fq = Fq
        self.Fqh = self.fft(Fq)
    def set_q_qtar_Fq(self, q, qtar, Fq):
        self.q=q
        self.qtar=qtar
        self.Fq=Fq

    def convmtx(self, v, n):
        ...
        """
        N = len(v) + 2*n - 2
        xpad = np.concatenate([np.zeros(n-1,dtype=complex), v[:], np.zeros(n-1,dtype=complex)])
        X = np.zeros((len(v)+n-1, n),dtype=complex)
