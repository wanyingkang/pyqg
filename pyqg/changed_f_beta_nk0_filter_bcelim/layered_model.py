...
#        self.a = np.ma.masked_invalid(a).filled(0.)
    ...
    def _initialize_inversion_matrix(self):
        ...
        if (self.nz==2):
            ...
            # give up the equation for high l wavenumber, located at int(self.nl/2)-1 and int(self.nl/2)
            #    1. NP: p|_{NP}=0
            #        For all k, wave#k component has no Amp at NP.
            M[:,:,int(self.nl/2)-1,:,:]=(Iz[:,:,np.newaxis,np.newaxis])*((np.exp(2*pi*1j*self.ll*(self.ny-1)/self.nl)/self.nl)[np.newaxis,np.newaxis,:,np.newaxis])
            ...
            M[:,:,int(self.nl/2),:,1:]=(Iz[:,:,np.newaxis,np.newaxis])*(1/self.nl)
            #    3. let -nl/2 and nl/2-1 mode has no contribution to other equations:
        ...
        b [:,:,0,0]=0.+0j
        b [:,:,int(self.nl/2)-1,:]=self.a[:,:,int(self.nl/2)-1,:,0]
        b [:,:,int(self.nl/2),:]=self.a[:,:,int(self.nl/2),:,0]
        self.a[:,:,:,:,0]=b
