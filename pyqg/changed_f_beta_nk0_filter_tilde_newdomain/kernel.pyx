    ...
    cdef void __do_advection(self) nogil:
        ...
        for k in range(self.nz):
            for j in prange(self.ny, nogil=True, schedule='static',
                      ...
                      num_threads=self.num_threads):
                for i in range(self.nx):
                    self.uq[k,j,i] = (self.u[k,j,i]+self.Ubg[k]) * self.q[k,j,i] / (self.coslat[j,i])**2 * exp(self.z[k]/2./self.H0) 
                    self.vq[k,j,i] = self.v[k,j,i] * self.q[k,j,i] / (self.coslat[j,i])**2 * exp(self.z[k]/2./self.H0)
